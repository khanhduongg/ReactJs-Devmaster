...
export default class App extends Component {
  ...
  handleonAddorEditView = (toggle,actionName,student)=>{(
    this.setState({
      isToggle:toggle,
  ...
  render() {
    ...
    let elementForm =this.state.isToggle===true?
          <Form renderActioneName={this.state.actionName}
                renderStundent={this.state.student} /> :"";
    return (
      <div className="row">
        <Title />
        <Control  onAddorEditView={this.handleonAddorEditView}/>
        <ListStudents 
                  onHandleView={this.onHandleView}        
                  renderStundents ={students} 
        />
        {elementForm}
      </div>
    );
